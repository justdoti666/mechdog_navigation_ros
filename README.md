...
将 `mechdog_navigation` 纯算法库（SensorFusion + PathPlanner）封装为 ROS2 节点，作为机械狗巡检的**局部安全层**，与师兄的 `quadruped_ws` 全局栈（Nav2 + 激光雷达 + AMCL + 栅格地图 + 安全闸门 + STM32 底盘桥）通过话题对接，构成完整的三库系统。
...
| **quadruped_ws**（师兄栈） | 师兄侧 | 全局层：Nav2 导航、cartographer/AMCL 建图定位、lidar 避障、语音/云台/热成像、双层速度闸门、STM32H723 串口底盘桥 |
...
- 师兄侧双层闸门 + 状态新鲜度（`/safety/state`、`/robot/state` 1s 超时即零速，fail-closed）是最终安全兜底。
- 本包 10Hz 周期发送（>2Hz，满足师兄 0.5s 超时要求）。

### 底盘桥接两种接法（二选一）

1. **本包 Stm32ChassisBridge**（`bridge_type:=stm32`）：本包直接通过串口发 21 字节帧（`AA 55 0x01 0x10 + 4×float32 LE RPM + 和校验`，差速运动学轮径 0.0645m/轮距 0.256m），协议与师兄 `wheel_board_bridge_node.py` 一致。
2. **师兄 wheel_board_bridge_node 接管**（推荐，联调默认）：本包保持 `bridge_type:=simulated`，由师兄栈统一管串口，避免双写。
...
| 话题 | 类型 | 方向 | 说明 |
|---|---|---|---|
| `/unsafe/cmd_vel` | geometry_msgs/Twist | 发布 | safety_node 产出速度指令 → 师兄安全闸门输入（默认；`cmd_vel_topic` 可改） |
| `/cmd_vel` | geometry_msgs/Twist | 订阅 | chassis_bridge_node 消费, 发送到底盘（师兄 wheel_board_bridge 或本包 stm32 桥） |
| `/fusion_result` | std_msgs/String (JSON) | 发布 | 融合结果 JSON（环境/悬崖/前方距离/动作/权重/速度） |
| `/scan` | sensor_msgs/LaserScan | 订阅 | 师兄雷达（sensor_data QoS, 当前缓存预留, 不参与融合） |

## 对接注意

- **安全闸门**：本包发布 `/unsafe/cmd_vel`，由师兄 `cmd_vel_safety_gate_node` 统一安全检查（estop/超时/限幅）后转发 `/cmd_vel`。若想绕过闸门直发（仅测试）：`--ros-args -p cmd_vel_topic:=/cmd_vel`。
- **底盘通信**：`chassis_bridge_node` 订阅 `/cmd_vel`。默认 `bridge_type:=simulated`；真机 `bridge_type:=stm32` 时本包直接串口发 21 字节帧（协议同师兄）。**联调默认让师兄 `wheel_board_bridge_node` 管串口，本包保持 simulated**，避免双写。
...
- [x] 对接师兄闸门（/unsafe/cmd_vel + sensor_data QoS）
- [ ] ROS2 版本确认（回学校后）
